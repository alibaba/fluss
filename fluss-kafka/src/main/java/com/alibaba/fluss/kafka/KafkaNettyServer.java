...
public final class KafkaNettyServer implements RpcServer {
    ...
    public void start() throws IOException {
        ...
        int ioThreads = conf.getInt(ConfigOptions.NETTY_SERVER_NUM_WORKER_THREADS);
        this.acceptor = NettyUtils.newEventLoopGroup(1, "fluss-kafka-netty-acceptor");
        this.worker = NettyUtils.newEventLoopGroup(ioThreads, "fluss-kafka-netty-worker");
    ...
    public CompletableFuture<Void> closeAsync() {
        ...
        CompletableFuture<Void> channelShutdownFuture = shutdownChannel(serverChannel);
        CompletableFuture<Void> acceptorShutdownFuture = shutdownGroup(acceptor);
        CompletableFuture<Void> selectorShutdownFuture = shutdownGroup(worker);
        return CompletableFuture.allOf(
                acceptorShutdownFuture, selectorShutdownFuture, channelShutdownFuture);
    }
    ...
    class KafkaChannelInitializer extends ChannelInitializer<SocketChannel> {
        ...
        protected void initChannel(SocketChannel ch) throws Exception {
            // TODO Maybe throttle the connection if the server is not ready.
