    ...
    public void updateClusterMetadata(ClusterMetadata clusterMetadata) {
        inLock(
                ...
                () -> {
                    ...
                    Map<TablePath, Long> newTableIdByPath = new HashMap<>();
                    List<TablePath> deletedTablePath = new ArrayList<>();
                    ...
                    for (TableMetadata tableMetadata : clusterMetadata.getTableMetadataList()) {
                        ...
                        if (tableId != TABLE_DURATION_DELETE_ID) {
                            newTableIdByPath.put(tablePath, tableId);
                        } else {
                            deletedTablePath.add(tablePath);
                        }
                    }

                    for (TablePath tablePath : deletedTablePath) {
                        tableIdByPath.remove(tablePath);
                    }
                    tableIdByPath.putAll(newTableIdByPath);
                    ...
                    Map<PhysicalTablePath, Long> newPartitionsIdByPath = new HashMap<>();
                    List<PhysicalTablePath> deletedPhysicalTablePath = new ArrayList<>();
                    ...
                    for (PartitionMetadata partitionMetadata :
                        ...
                        PhysicalTablePath physicalTablePath =
                                PhysicalTablePath.of(
                                        tablePath, partitionMetadata.getPartitionName());
                        long partitionId = partitionMetadata.getPartitionId();
                        if (partitionId != PARTITION_DURATION_DELETE_ID) {
                            newPartitionsIdByPath.put(physicalTablePath, partitionId);
                        } else {
                            deletedPhysicalTablePath.add(physicalTablePath);
                        }
                    }

                    for (PhysicalTablePath physicalTablePath : deletedPhysicalTablePath) {
                        partitionsIdByPath.remove(physicalTablePath);
                    }
                    partitionsIdByPath.putAll(newPartitionsIdByPath);
