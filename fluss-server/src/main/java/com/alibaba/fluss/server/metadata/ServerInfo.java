...
public class ServerInfo {
    ...
    public ServerInfo(Integer id, List<Endpoint> endpoints) {
        this.id = id;
        this.endpointMap =
                endpoints.stream()
    ...
    public @Nullable ServerNode toTableServerNode(String listenerName) {
        if (endpoint(listenerName) == null) {
            return null;
        }
        Endpoint endpoint = endpoint(listenerName);
        return new ServerNode(id, endpoint.getHost(), endpoint.getPort(), ServerType.TABLET_SERVER);
    }

    public @Nullable ServerNode toCoordinatorServerNode(String listenerName) {
        if (endpoint(listenerName) == null) {
            return null;
        }
        Endpoint endpoint = endpoint(listenerName);
